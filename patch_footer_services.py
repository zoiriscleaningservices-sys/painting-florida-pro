...
def build_mega_footer(hubs_list, silo_prefix="/"):
    """
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-12 lg:gap-8 mb-16 sm:mb-24">
                            ...
                            >
                                ...
                                <ul className="space-y-4">
                                    <li>
                                        <a href="{silo_prefix}residential-painting/" className="text-brand-dark/70 hover:text-brand-orange hover:translate-x-1 transition-all inline-block flex items-center gap-2 text-sm">
                                            <Icon name="ChevronRight" className="w-3 h-3 text-brand-orange" />
                                            Residential Painting
                                        </a>
                                    </li>
                                    <li>
                                        <a href="{silo_prefix}commercial-painting/" className="text-brand-dark/70 hover:text-brand-orange hover:translate-x-1 transition-all inline-block flex items-center gap-2 text-sm">
                                            <Icon name="ChevronRight" className="w-3 h-3 text-brand-orange" />
                                            Commercial Property
                                        </a>
                                    </li>
                                    <li>
                                        <a href="{silo_prefix}interior-painting/" className="text-brand-dark/70 hover:text-brand-orange hover:translate-x-1 transition-all inline-block flex items-center gap-2 text-sm">
                                            <Icon name="ChevronRight" className="w-3 h-3 text-brand-orange" />
                                            Interior Painting
                                        </a>
                                    </li>
                                    <li>
                                        <a href="{silo_prefix}exterior-painting/" className="text-brand-dark/70 hover:text-brand-orange hover:translate-x-1 transition-all inline-block flex items-center gap-2 text-sm">
                                            <Icon name="ChevronRight" className="w-3 h-3 text-brand-orange" />
                                            Exterior Painting
                                        </a>
                                    </li>
                                    <li>
                                        <a href="{silo_prefix}epoxy-flooring/" className="text-brand-dark/70 hover:text-brand-orange hover:translate-x-1 transition-all inline-block flex items-center gap-2 text-sm">
                                            <Icon name="ChevronRight" className="w-3 h-3 text-brand-orange" />
                                            Epoxy Flooring
                                        </a>
                                    </li>
                                    <li>
                                        <a href="{silo_prefix}pressure-washing/" className="text-brand-dark/70 hover:text-brand-orange hover:translate-x-1 transition-all inline-block flex items-center gap-2 text-sm">
                                            <Icon name="ChevronRight" className="w-3 h-3 text-brand-orange" />
                                            Pressure Washing
                                        </a>
                                    </li>
        ...
        for file in files:
            if file.endswith('.html'):
                ...
                try:
                    ...
                    if match:
                        # Calculate the silo_prefix so local links are insulated
                        rel_path = os.path.relpath(root, BASE_DIR)
                        parts = rel_path.split(os.sep)
                        
                        silo_prefix = "/"
                        if parts[0] != '.':
                            # We are inside a hub folder!
                            silo_prefix = f"/{parts[0]}/"

                        # 2. Build a unique mega footer specifically for this individual file (Randomizes the 5 cities)
                        new_mega_footer = build_mega_footer(all_hubs, silo_prefix)
